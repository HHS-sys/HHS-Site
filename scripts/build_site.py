...
_FIGURE_BLOCK = re.compile(r"<figure(?P<attrs>[^>]*)>(?P<body>.*?)</figure>", re.IGNORECASE | re.DOTALL)
...
def media_orientation_class(source: str) -> str:
    """Return a reusable class that reflects a verified local image's shape."""
    dimensions = local_raster_dimensions(source)
    if not dimensions:
        return ""
...
def add_media_orientation_classes(markup: str) -> str:
    """Annotate image figures so mixed galleries can preserve each real aspect ratio."""
    def annotate(match: re.Match[str]) -> str:
        ...
        source = _attribute_value(image_match.group(0), "src")
        orientation = media_orientation_class(source) if source else ""
        if not orientation:
            return match.group(0)
        ...
        if class_match:
            ...
            if orientation in classes:
                return match.group(0)
            replacement = f'class={class_match.group(1)}{class_match.group(2)} {orientation}{class_match.group(1)}'
            attrs = attrs[:class_match.start()] + replacement + attrs[class_match.end():]
        else:
            attrs += f' class="{orientation}"'
        return f"<figure{attrs}>{body}</figure>"

    return _FIGURE_BLOCK.sub(annotate, markup)
...
def page(title: str, description: str, path: str, image: str, current: str, body: str, body_class: str = "", *, indexable: bool = True, image_alt: str | None = None) -> str:
    ...
    return add_intrinsic_image_attributes(add_media_orientation_classes(document))
...
def project_spotlight(slug: str) -> str:
    ...
    return f"""
      <section class="section section-stone project-spotlight"><div class="wrap editorial-grid">
        <div class="editorial-media {media_orientation_class(f'/{image}')} reveal"><img src="/{image}" alt="{html.escape(alt, quote=True)}" loading="lazy" decoding="async"><span>Genuine local project</span></div>
...
def service_card(slug: str, *, compact: bool = False, variant: int = 0) -> str:
    ...
    <a class="{class_name} reveal" href="{service_url(slug)}">
      <span class="service-card-media" style="--card-image:url('/{image}')"><img src="/{image}" alt="{html.escape(image_alt, quote=True)}" loading="lazy" decoding="async"></span>
...
def gallery_figure(source: str, alt: str, caption: str) -> str:
    """Render a service-gallery image with its verified source orientation."""
    dimensions = local_raster_dimensions(f"/{source}")
    classes = ["reveal"]
    if dimensions and dimensions[0] > dimensions[1]:
        classes.append("gallery-landscape")
    class_name = " ".join(classes)
    return (
        f'<figure class="{class_name}"><img src="/{source}" '
...
def homepage() -> str:
    ...
    local_proof = f"""
      <section class="section section-stone local-proof-section">
        <div class="wrap">
          {section_heading("Recent work across London", "Finished results, backed by the story behind them", "A Medway upper level, two very different Westmount projects and a reworked Melrose-area bathroom—four genuine transformations with a documented story.")}
          <div class="story-card-grid local-proof-grid">
            ...
            <a class="story-card reveal" href="/projects/glass-block-bathroom-conversion/"><img src="/bathroom-walnut-vanity-after.jpg" alt="Completed Westmount bathroom with a glass shower, walnut vanity and original glass-block window" loading="lazy"><span><small>Westmount · completed bathroom</small><strong>A bathroom rebuilt behind the finish</strong><b>Jetted-tub conversion, careful coordination, tile and complete finish work <i aria-hidden="true">↗</i></b></span></a>
          ...
          <div class="testimonial-grid" aria-label="Homeowner feedback connected to documented projects">
            <blockquote class="testimonial-card reveal"><p>“I absolutely love their work. They are professional, super customer focused and know what they are doing.”</p><footer><strong>Medway homeowner</strong><span>Verified Google review · flooring &amp; storage transformation</span><a href="/projects/medway-flooring-storage/">See the documented work <i aria-hidden="true">↗</i></a></footer></blockquote>
            <blockquote class="testimonial-card reveal"><p>“VERY easy to work with, excellent customer service, very responsive, met our needs 100%.”</p><footer><strong>Pond Mills homeowner</strong><span>Verified Google review · flooring &amp; connected repairs</span><a href="/projects/pond-mills-home-repairs/">See the documented work <i aria-hidden="true">↗</i></a></footer></blockquote>
          </div>
    ...
    <main id="main">
      ...
      <section class="section section-charcoal">
        <div class="wrap editorial-grid">
          <div class="editorial-media {media_orientation_class('/hilltop-kitchen-sink.jpg')} reveal"><img src="/hilltop-kitchen-sink.jpg" alt="Finished Hilltop kitchen sink, counter and backsplash details" loading="lazy" decoding="async"><span>Genuine completed work</span></div>
...
def services_page() -> str:
    card_variants = {"structural-layout": 3}
    cards = "".join(service_card(slug, compact=True, variant=card_variants.get(slug, 1)) for slug in SERVICE_DISPLAY_ORDER)
    body = f"""
    {project_story_hero("project-129.jpg", "Completed kitchen renovation", "Renovation & repair services", "One clear scope for the work your property needs", "Renovations, repairs and property improvements are planned together—from the first visible problem to the details that make the completed result hold together.")}
    <main id="main">
...
def bathroom_showcase() -> str:
    ...
    <section class="section section-paper bathroom-showcase">
      <div class="wrap">
        ...
        <div class="secondary-case-grid">
          <article class="secondary-case-card reveal" id="hilltop-bathrooms">
            <figure><img src="/hilltop-green-tile-after.jpg" alt="Completed Hilltop bathroom with a marble-look tub surround" loading="lazy"><figcaption>Hilltop bathroom result</figcaption></figure>
            <div><p class="eyebrow">Two Hilltop bathrooms</p><h3>Different rooms, connected by one finish standard</h3><p>An original green-tile surround upstairs and an older basement shower were rebuilt as part of the wider Hilltop transformation.</p><a class="text-link dark-link" href="/projects/hilltop-home-transformation/#hilltop-bathrooms">See both bathroom stories <span aria-hidden="true">↗</span></a></div>
          </article>
          <article class="secondary-case-card reveal" id="frosted-window-repair">
            <figure><img src="/project-152.jpg" alt="New wall board and compound around the unchanged frosted bathroom window" loading="lazy"><figcaption>Wall rebuilding stage</figcaption></figure>
            <div><p class="eyebrow">The work behind the finish</p><h3>The same window anchors every repair stage</h3><p>Tile removal opened the wall for insulation and plumbing access before new board and compound rebuilt the surfaces around the unchanged window and shower.</p><a class="text-link dark-link" href="/services/drywall-ceiling-repair/">Explore wall and ceiling repair <span aria-hidden="true">↗</span></a></div>
          </article>
        </div>
      </div>
    </section>
      ...
      <div class="wrap">
        ...
        <article class="case-study reveal">
          ...
          <div class="case-study-media case-study-media-three">
            <figure><img src="/salon-restored-wall-detail.jpg" alt="Completed repaired salon wall with its original alarm and ceiling details retained" loading="lazy"><figcaption>Completed restoration detail</figcaption></figure>
        ...
        <div class="secondary-case-grid">
          <article class="secondary-case-card reveal">
            <figure><img src="/project-048.jpg" alt="Fitness facility after the completed LED lighting upgrade" loading="lazy"><figcaption>Completed LED upgrade</figcaption></figure>
            <div><p class="eyebrow">Fitness facility</p><h3>Better light for a space built around movement</h3><p>Older box-style fluorescent fixtures were replaced with streamlined LED lighting for clearer, more consistent light across the gym floor.</p></div>
          </article>
          <article class="secondary-case-card reveal">
            ...
            <div><p class="eyebrow">Office kitchen · London</p><h3>A compact staff space rebuilt to work better</h3><p>Cabinets, counter, sink, wall access and finish work came together in the existing footprint.</p><a class="text-link dark-link" href="/projects/kitchen-renewal/">See the full kitchen sequence <span aria-hidden="true">↗</span></a></div>
          </article>
        </div>
      </div>
    </section>
...
def service_page(slug: str) -> str:
    ...
    spotlight_images = {
        "flooring": "medway-floor-door-transition.jpg",
        "structural-layout": "melrose-wall-hung-toilet-progress.jpg",
        "handyman-repairs": "westmount-porch-after-day.jpg",
        "decks-exterior": "project-104.jpg",
        "basements": "pond-mills-basement-floor-after.jpg",
        "kitchens": "hyde-park-kitchen-after.jpg",
        "bathrooms": "melrose-bathroom-after.jpg",
        "drywall-ceiling-repair": "melrose-exercise-room-after.jpg",
        "water-damage": "blackfriars-investigation.jpg",
    }
    excluded_gallery_images = {item["hero"], spotlight_images.get(slug)}
    gallery_items = [entry for entry in item["gallery"] if entry[0] not in excluded_gallery_images][:6]
    gallery = "".join(gallery_figure(src, alt, caption) for src, alt, caption in gallery_items)
    faqs = "".join(f'<details class="reveal"><summary>{question}</summary><p>{answer}</p></details>' for question, answer in item["faq"])
    related_variants = {
        # Keep all three cards visually distinct on the Decks & Exterior page.
        "decks-exterior": (3, 2, 3),
        # Avoid repeating the salon result already used in the commercial showcase.
        "commercial": (2, 3, 1),
        # Keep the flooring hero distinct from its connected-service cards.
        "flooring": (3, 1, 1),
    }.get(slug, tuple(range(2, 2 + len(item["related"]))))
    related = "".join(
        service_card(related_slug, compact=True, variant=variant)
        for related_slug, variant in zip(item["related"], related_variants)
    )
    ...
    motion_section = ""
    if slug == "drywall-ceiling-repair":
        motion_section = f"""
      <section class="section section-paper"><div class="wrap">{section_heading("Work in motion", "Drywall finishing around a new pot-light layout", "This short, silent clip is clearly labelled as work in progress. It shows the ceiling during the repair and finishing stages—not a claimed completed result.")}<div class="video-grid video-grid-single"><figure class="work-video reveal"><video controls playsinline preload="none" poster="/drywall-potlight-progress-poster.jpg" aria-label="Short work-in-progress clip showing drywall finishing around pot-light openings"><source src="/drywall-potlight-progress.mp4" type="video/mp4">Your browser does not support embedded video.</video><figcaption><strong>Ceiling finishing in progress</strong><span>Drywall preparation and compound work around the pot-light openings</span></figcaption></figure></div></div></section>
    """
    if slug in {"bathrooms", "commercial"}:
        gallery_section = ""
    ...
    <main id="main">
      ...
      {showcase}
      {gallery_section}{motion_section}
      {spotlight}
...
def hyde_park_kitchen_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("project-119.jpg", "Completed Hyde Park kitchen viewed across the peninsula toward the pantry and dining area", "Hyde Park · London, Ontario", project_name, "A resourceful update kept the kitchen’s useful foundations, then improved storage, appliance flow and the surfaces the homeowners touch every day.")}
    <main id="main">
      {breadcrumbs(project_name)}
    ...
    return page("Hyde Park Kitchen Renewal | Hekman Home Services", PROJECT_DETAILS["/projects/hyde-park-kitchen-renewal/"]["description"], "/projects/hyde-park-kitchen-renewal/", "project-119.jpg", "projects", body, "project-story-page")
def blackfriars_project_page() -> str:
    ...
    <main id="main">
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("From symptom to restored room", "Open carefully, understand fully, rebuild in the right order", "The sequence follows the first opening, deeper investigation, a structural detail and the rebuilding stage. The completed room is shown above as the final result.")}<div class="story-mosaic story-mosaic-blackfriars">
...
def hilltop_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("hilltop-kitchen-wide.jpg", "Completed Hilltop kitchen and island", "Hekman project story · London, Ontario", "Hilltop: one home, one clear point of view", "A whole-home transformation connecting the kitchen, bathroom, lower level, entry, stairs, flooring and finish details into a cohesive result.")}
    <main id="main">
      {breadcrumbs(project_name)}
      ...
      <section class="section section-paper" id="hilltop-bathrooms"><div class="wrap">{section_heading("Two bathroom transformations", "The room details make each sequence unmistakable.", "A sloped ceiling and ornate mirror identify the upstairs bathroom. Downstairs, the shower position, vanity wall and compact footprint carry through from the older room to the finished glass enclosure.")}
        <div class="comparison-grid comparison-grid-two comparison-landscape">
...
def medway_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("medway-finished-room.jpg", "Completed Medway room with cool gray-brown plank flooring and finished baseboards", "Medway project story · London, Ontario", "More storage. Better flow. One seamless upper level", "What began as a flooring update grew into a practical transformation of three rooms, with relocated and new closets, coordinated trim and clean transitions throughout.")}
    <main id="main">
      {breadcrumbs(project_name)}
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("Before, during and complete", "The real work is visible in the transitions", "Four verified photographs continue the project below; the completed room is shown in the hero above. The sequence includes the relocated closet opening and the exact plank colour at the doorway. No installation photograph from another flooring project is used here.")}<div class="story-mosaic story-mosaic-medway">
        <figure><img src="/medway-closet-before.jpg" alt="Medway room before flooring, with carpet and closet work already underway" loading="lazy"><figcaption>Before flooring: closet work underway</figcaption></figure>
      ...
      <section class="section section-stone"><div class="wrap"><div class="story-copy-panel reveal"><p class="eyebrow">Budget-conscious collaboration</p><h2>More function, with the spending focused where it mattered</h2><p>The homeowner supplied the flooring. Hekman Home Services provided the trim and doors at cost, helping the larger storage and finish scope stay within the homeowner’s budget.</p><p>The result improves daily storage, creates a more connected upper level and adds a cleaner finish that supports future resale appeal.</p><a class="text-link dark-link" href="/services/flooring/">Explore flooring installation <span aria-hidden="true">↗</span></a></div></div></section>
      <section class="section section-paper"><div class="wrap testimonial-feature reveal"><p class="eyebrow">Homeowner feedback</p><blockquote>“I absolutely love their work. They are professional, super customer focused and know what they are doing.”</blockquote><p>The homeowner said the finished upstairs is stunning, highlighted the clean workday and plans to call Hekman Home Services for future projects.</p><cite>Verified Google review · Anonymous Medway homeowner</cite></div></section>
      <section class="section section-charcoal"><div class="wrap">{section_heading("Connected services", "Flooring, storage and finish carpentry in one plan.", "The room works because the floor, closets, doors and trim were considered together.")}<div class="service-grid related-grid">{service_card("flooring", compact=True, variant=1)}{service_card("structural-layout", compact=True, variant=4)}{service_card("handyman-repairs", compact=True, variant=0)}</div></div></section>
    ...
    return page("Medway Flooring & Storage Transformation | Hekman", PROJECT_DETAILS["/projects/medway-flooring-storage/"]["description"], "/projects/medway-flooring-storage/", "medway-finished-room.jpg", "projects", body, "project-story-page")
def westmount_porch_project_page() -> str:
    project_name = "A Westmount Porch and Entry, Modernized by Neighbours"
    body = f"""
    {project_story_hero("westmount-porch-after-night.jpg", "Finished Westmount porch and entry illuminated at night", "Westmount project story · London, Ontario", "A more modern welcome, built by neighbours", "A separate porch and entry revitalization for a repeat Westmount customer and neighbour who has also trusted Hekman Home Services with several handyman projects.")}
    <main id="main">
      {breadcrumbs(project_name)}
...
def westmount_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("westmount-transformation-blue-wall-flooring.jpg", "Completed Westmount living-space phase with plank flooring, pot lights and a deep-blue feature wall", "Ongoing Westmount transformation · London, Ontario", "A home transformation built in thoughtful phases", "This is an ongoing project, planned and completed in stages around the clients’ life, timing and budget. Every finished phase is designed to connect with what came before—and what is still ahead.")}
    <main id="main">
      {breadcrumbs(project_name)}
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("Work completed in connected phases", "From demolition to a kitchen still in progress.", "Five selected photographs continue the sequence below; the completed living-space phase is shown in the project hero above. The kitchen remains a current progress view, not a final after photograph.")}<div class="story-mosaic story-mosaic-westmount-phased">
...
def salon_project_page() -> str:
    ...
    <main id="main">
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("Completed result first", "A bright working salon—and the repair sequence behind it", "The finished wall leads the story. Supporting photographs document the investigation, what was found and the rebuilding work that made the result possible.")}<div class="story-mosaic story-mosaic-salon">
        <figure class="story-feature"><img src="/salon-after-1.jpg" alt="Broader view of the anonymous London salon after the repair, with protective tape still visible" loading="lazy"><figcaption>Broader room context after repair</figcaption></figure>
      ...
      <section class="section section-stone"><div class="wrap"><div class="story-copy-panel reveal"><p class="eyebrow">A repair planned around a business</p><h2>The room still had a job to do</h2><p>Access, staff, clients and normal operating needs all shape commercial restoration. The work had to address the affected construction while protecting the salon’s established layout, lighting and identity.</p><p>Once the affected material was removed, the wall and ceiling could be rebuilt, prepared and finished as one connected surface.</p><a class="text-link dark-link" href="/services/commercial/">Explore commercial repairs <span aria-hidden="true">↗</span></a></div></div></section>
      <section class="section section-charcoal"><div class="wrap">{section_heading("Related expertise", "Investigation, rebuilding and finish work in one clear story", "Commercial access, restoration sequencing and precise drywall finishing all mattered to this result.")}<div class="service-grid related-grid">{service_card("commercial", compact=True, variant=0)}{service_card("water-damage", compact=True, variant=1)}{service_card("drywall-ceiling-repair", compact=True, variant=0)}</div></div></section>
...
def pond_mills_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("pond-mills-flooring-walkthrough.jpg", "Completed Pond Mills flooring carried from carpet into the adjoining room", "Pond Mills · London, Ontario", "One repair list. A much closer look.", "When a London home had not sold, the homeowner asked for help. A closer inspection connected the visible interior work with moisture concerns and an exterior water-management plan.")}
    <main id="main">
      {breadcrumbs(project_name)}
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("Same kitchen, clear result", "The floor now carries cleanly into the room", "The doorway, oak cabinetry, white range and threshold make the change easy to compare.")}<div class="comparison-grid comparison-grid-two comparison-portrait">
    ...
    return page("Pond Mills Home Repairs & Flooring | Hekman", PROJECT_DETAILS["/projects/pond-mills-home-repairs/"]["description"], "/projects/pond-mills-home-repairs/", "pond-mills-flooring-walkthrough.jpg", "projects", body, "project-story-page")
...
def multi_unit_deck_project_page() -> str:
    ...
    <main id="main">
      ...
      <section class="section section-charcoal"><div class="wrap">{section_heading("Before, during and complete", "The same brick row through the work", "The building, bay projections, deck spacing and rear elevations anchor every stage to the same anonymous property.")}<div class="story-mosaic story-mosaic-decks">
        ...
        <figure class="story-panorama"><img src="/multi-unit-deck-completed-row.jpg" alt="Completed row of renewed rear decks viewed along the anonymous multi-unit property" loading="lazy"><figcaption>Completed row of renewed decks</figcaption></figure>
      </div></div></section>
...
def kitchen_renewal_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("kitchenette-after-wide.jpg", "Completed office kitchen with walnut-look cabinetry and gray counter", "Commercial kitchen project · London, Ontario", "A compact office kitchen, rebuilt around the work it needs to do", "Old cabinetry came out, wall and plumbing access were addressed, and the staff space was rebuilt with clean-lined storage, a new counter, sink and finish details.")}
    <main id="main">
      {breadcrumbs(project_name)}
...
def popcorn_project_page() -> str:
    ...
    body = f"""
    {project_story_hero("project-017.jpg", "Skim coating underway with the room protected during smooth-ceiling work", "Ceiling transformation · London, Ontario", "From popcorn texture to a clean, modern ceiling", "A preparation-heavy process documented from the original texture through sanding, skim coats, surface checks and primer.")}
    <main id="main">
      {breadcrumbs(project_name)}
    ...
    return page("Popcorn Ceiling Transformation London ON | Hekman Home Services", "See a Hekman Home Services popcorn ceiling project in London, Ontario, from the original textured ceiling through sanding, skim coating and primer.", "/projects/popcorn-ceiling-transformation/", "project-017.jpg", "projects", body, "project-story-page")
def glass_block_bathroom_project_page() -> str:
    project_name = "Westmount Bathroom Conversion: Jetted Tub to Glass Shower"
    body = f"""
    {project_story_hero("bathroom-vanity-toilet-finish.jpg", "Completed walnut vanity, toilet and gray floor in the Westmount bathroom", "Westmount · completed bathroom", "A Westmount bathroom rebuilt behind the finish", "What began as a jetted-tub-to-shower conversion became a deeper rebuild when demolition exposed an unusual combination of concealed conditions. The issues were documented, the appropriate specialists were brought in and the room was completed with a new shower system, tile, fixtures and carefully blended finishes.")}
    <main id="main">
      {breadcrumbs(project_name)}
      <section class="section section-paper"><div class="wrap service-intro story-summary"><div class="reveal"><p class="eyebrow">The goal</p><h2>Replace the oversized tub with a brighter, more useful shower</h2><p>The original jetted tub and deep tiled platform filled the window end of the bathroom. The plan removed that assembly, retained the glass-block window and rebuilt the footprint as a tiled shower with a sliding glass enclosure.</p><p>Once the wall and floor were opened, the project needed more than a fixture swap. Each concealed condition was documented and handled before the new wet-area assembly and final finishes went in.</p></div><ul class="scope-list reveal"><li>Jetted tub and tiled-platform removal</li><li>Concealed-condition documentation and trade coordination</li><li>Backing, support and insulation restoration</li><li>Shower preparation and water management</li><li>Tile, enclosure, vanity, toilet and fixtures</li><li>Blended drywall, new trim and repainting</li></ul></div></section>
      <section class="section section-charcoal"><div class="wrap">{section_heading("The conversion sequence", "The same window, with a completely different wet area", "Four verified views follow this one Westmount bathroom from the existing jetted tub to the completed shower.")}<div class="story-mosaic story-mosaic-bathroom">
        ...
        <figure><img src="/bathroom-glass-block-open-wall.jpg" alt="Tub removed with the floor and wall cavities opened after concealed conditions were discovered" loading="lazy"><figcaption>Wall and floor opened for the rebuild</figcaption></figure>
      ...
      <section class="section section-stone"><div class="wrap">{section_heading("The transformation in motion", "From the jetted tub to the finished glass shower", "This 14-second, silent edit keeps the useful stages: the original room, opened assembly, water-management preparation, tile, glass installation and the completed bathroom.")}<div class="video-grid video-grid-single"><figure class="work-video reveal"><video controls playsinline preload="none" poster="/bathroom-transformation-poster.jpg" aria-label="Short transformation video showing the Westmount jetted tub converted into a glass shower"><source src="/bathroom-glass-block-transformation.mp4" type="video/mp4">Your browser does not support embedded video.</video><figcaption><strong>Tub-to-shower transformation</strong><span>Original layout, genuine rebuild stages and the completed bathroom</span></figcaption></figure></div></div></section>
      <section class="section section-charcoal"><div class="wrap">{section_heading("What demolition revealed", "An unusual combination, handled one issue at a time", "These conditions were unusual and are not what every bathroom renovation uncovers. The important part was to pause in the affected areas, document what was visible and resolve each concern before closing the assembly.")}<div class="proof-grid story-step-grid"><article class="proof-card reveal"><h3>Concealed electrical conditions</h3><p>Hidden junction boxes and wiring were documented and referred to the appropriate licensed electrical trade before the wall was closed.</p></article><article class="proof-card reveal"><h3>Window-wall support and insulation</h3><p>Missing backing and support around part of the existing window assembly, along with areas without insulation, were restored as the surrounding wall was rebuilt.</p></article><article class="proof-card reveal"><h3>A paper-wasp nest in the wall</h3><p>A large nest was found inside the opened cavity. Work paused in that area while a pest-control specialist handled it safely.</p></article><article class="proof-card reveal"><h3>The complete room, not only the shower</h3><p>The finished scope brought together shower preparation and waterproofing, tile, glass, vanity, toilet, plumbing connections, blended drywall, new trim and paint.</p></article></div><p class="story-role-note reveal">Hekman documented the concealed conditions, coordinated access and completed the surrounding bathroom rebuild. A pest-control specialist handled the paper-wasp nest, and the electrical condition was referred to the appropriate licensed electrical trade before the assembly was closed.</p><div class="testimonial-feature project-feedback reveal"><p class="eyebrow">Homeowner feedback</p><blockquote>“Love! Thank you so much!” and “You guys deserve it!”</blockquote><cite>Anonymous Westmount homeowner</cite></div></div></section>
    ...
    return page("Westmount Bathroom Conversion | Jetted Tub to Glass Shower | Hekman", "See a Westmount jetted-tub-to-shower conversion, including unusual concealed conditions, specialist coordination and the complete tiled bathroom finish.", "/projects/glass-block-bathroom-conversion/", "bathroom-vanity-toilet-finish.jpg", "projects", body, "project-story-page")
def projects_page() -> str:
    ...
    ordered_projects = sorted((item for item in PROJECTS if item[0] not in featured_sources), key=lambda item: priority.get(item[0], len(PROJECTS)))
    ...
    <main id="main">
      <section class="section section-paper"><div class="wrap">{section_heading("Project stories", "Six transformations worth exploring in detail", "Explore the original challenge, the decisions behind the work and the completed result in each project story.")}
        <div class="story-card-grid">
          ...
          <a class="story-card reveal" href="/projects/hyde-park-kitchen-renewal/"><img src="/project-119.jpg" alt="Completed Hyde Park kitchen viewed across the peninsula toward the pantry and dining area" loading="lazy"><span><small>Hyde Park · completed</small><strong>Renewed without starting over</strong><b>Cabinet refacing, pantry, appliance flow, dishwasher and new surfaces <i aria-hidden="true">↗</i></b></span></a>
          ...
          <a class="story-card reveal" href="/projects/glass-block-bathroom-conversion/"><img src="/bathroom-walnut-vanity-after.jpg" alt="Completed Westmount bathroom with gray tiled shower, walnut vanity and original glass-block window" loading="lazy"><span><small>Westmount · completed bathroom</small><strong>A bathroom rebuilt behind the finish</strong><b>Jetted-tub removal, unusual hidden conditions, shower system, tile and final finishes <i aria-hidden="true">↗</i></b></span></a>
